...
from torch.nn import Embedding, Linear, ModuleList, Sequential, Identity, Softmax
...
class SchNet(torch.nn.Module):
    def __init__(
        ...
        if self.sphere_dim is not None:
            self.lin2 = Linear(hidden_channels // 2, self.sphere_dim)
            self.lin3 = Linear(self.sphere_dim, self.out_dim)
        else:
            self.lin2 = Linear(hidden_channels // 2, self.out_dim)
    ...
    def forward(self, inputs) -> Tensor:
        ...
        Args:
            ...
            batch (torch.Tensor, optional): Batch indices assigning each atom
                ...
                (default: :obj:`None`)
        """
        z, pos, batch = inputs.z, inputs.pos, inputs.batch
        ...
        if self.sphere_dim is not None:
            return self.softmax(self.lin3(out)), out
        else: 
            return out
