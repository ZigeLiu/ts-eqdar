...
class NodeInit(MessagePassing):
    def __init__(self, hidden_channels, num_rbf, cutoff, cg_degree=1, max_z=100, activation=F.silu, proj_ln='',
        ...
        if self.cg_degree > 1:
            ...
            self.cg_linear = torch.nn.Linear(self.cg_degree*last_channel,last_channel)
    ...
    def forward(self, z, x, edge_index, edge_weight, edge_attr, cg_index=None):
        # remove self loops
        mask = edge_index[0] != edge_index[1]
        if not mask.all():
        ...
        if self.coarse_grain:
            assert cg_index is not None
            idx = cg_index.long().to(x_neighbors.device)   # [N]
            K   = int(idx.max()) + 1 # number of cg nodes 
            counts  = torch.bincount(idx, minlength=K)        # [K] cg degree list
            max_len = counts.max().item()
            n_atom_basis = x_neighbors.shape[1]
            padded = x_neighbors.new_zeros((K, max_len, n_atom_basis)) # all zero
            row = torch.repeat_interleave(torch.arange(K, device=x_neighbors.device), counts)
            col = torch.arange(max_len, device=x_neighbors.device).repeat(K) #[node number]
            mask = col < counts.reshape(1,-1).repeat(max_len,1).transpose(1,0).reshape(-1)             # [K, max_len]
            padded[row, col[mask]] = x_neighbors                     # scatter into padded
            x_neighbors = self.cg_linear(padded.reshape(K,-1))
